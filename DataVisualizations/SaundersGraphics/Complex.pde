class Complex{
 ...
 public Complex sqrt() {
        //±√r(cos(θ/2)+isin(θ/2))
        //re: √r(cos(θ/2), im: √rsin(θ/2)
        //return new Complex(sqrt((float)this.modulus())*cos((float)this.argument() / 2.0), sin((float)this.argument() / 2.0));
        //im() is glitchy while result could have two values
        return new Complex(Math.sqrt(this.modulus()) * Math.cos(this.argument() / 2.0), Math.sqrt(this.modulus()) * Math.sin(this.argument() / 2.0));
    }
