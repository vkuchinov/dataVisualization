        ...
        render() {
            ...
            return (
                ...
                <div style={{ width: '100%', height: '100%' }} ref={node => this.node = node} >
                    {
                        width && <Component width={Math.max(minWidth, min)} height={Math.max(minWidth, height)} {...props} />
                    }
                </div>
    ...
    componentDidMount() {
     ...
     _onMouseMove(e) {
        ...
        if(!this.toggle){
            ...
            this.setState({ x: e.pageX, y: e.pageY, target: e, tooltip: { target: e.target, bcolor: bcolor, top: e.pageY + 8, left: e.pageX + 8, display: hidden, html: html } });
...
export default Responsive(VennDiagram);